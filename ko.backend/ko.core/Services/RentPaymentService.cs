...
namespace ko.core.Services
{
    public class RentPaymentService : IRentPaymentService
    {
        ...
        {
            _logger.LogInformation(
                "Getting payment summary for student {0}", studentId);
            ...
            if (tenancy == null)
                throw new NotFoundException(
                    "Active tenancy not found for student", studentId);
        ...
        public async Task<RentPaymentDto?> AddAsync(AddRentPaymentDto dto)
        {
            ...
            var entity = new RentPayment
            {
                ...
                DueDate = new DateTime(
                    dto.DueDate.Year, dto.DueDate.Month, 1, 0, 0, 0, DateTimeKind.Utc),
        ...
        public async Task<RentPaymentDto> MarkAsPaidAsync(MarkRentPaidDto dto)
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "Receipt generated for payment {0}: {1}",
            ...
            await _notificationService.SendToUserAsync(
                ...
                message: $"Your rent payment of R {payment.Amount:N0} for " +
                         $"{tenancy.Property?.Title ?? "your property"} " +
                         "has been recorded successfully.",
                type: "payment");

            _logger.LogInformation(
                "Payment {0} marked as paid successfully", payment.Id);
        ...
        public async Task<bool> SendPaymentReminderAsync(SendReminderDto dto)
        {
            ...
            var totalOwed = overduePayments.Sum(p => p.Amount)
                               + pendingPayments.Sum(p => p.Amount);
            var propertyTitle = tenancy.Property?.Title is { Length: > 0 } t && t != "string"
                ? t : $"Property #{tenancy.PropertyId}";
    ...
    <div class='wrapper'>
        <div class='container'>
            ...
            <div class='body'>
                ...
                <p>This is a friendly reminder from your landlord regarding outstanding rent payments for:</p>
                <p><strong>{propertyTitle}</strong><br/>
                <span style='color:#666;font-size:13px;'>{propertyAddress}</span></p>
                <div class='amount-block'>
                    <p style='margin:0;font-size:13px;color:#666;'>Total Outstanding</p>
                    <div class='amount-value'>R {totalOwed:N0}</div>
                    <p style='margin:0;font-size:12px;color:#999;'>Please settle this amount as soon as possible.</p>
                </div>
                {(overduePayments.Any() ? $@"
                <div class='overdue-banner'>
                    ⚠️ You have <strong>{overduePayments.Count} overdue payment(s)</strong> that require immediate attention.
                </div>
                <p><strong>Overdue Payments:</strong></p>
                <table class='table'>
                    <tr>
                        <th>Period</th><th>Due Date</th><th>Amount</th><th>Status</th>
                    </tr>
                    {overdueRowsHtml}
                </table>" : "")}
                {(pendingPayments.Any() ? $@"
                ...
                <div style='text-align:center;'>
                    <p style='color:#666;font-size:13px;'>Please log in to your VeriStay dashboard to make your payment.</p>
                    <a href='{dashboardUrl}dashboard' class='button'>Pay Now on VeriStay</a>
                </div>
                <p style='font-size:12px;color:#999;margin-top:16px;text-align:center;'>
                    If you believe this is an error or have already made payment, please contact your landlord directly.
                </p>
            </div>
            <div class='footer'>
            ...
            await _notificationService.SendToUserAsync(
                ...
                message: $"You have outstanding rent of R {totalOwed:N0} for " +
                         $"{propertyTitle}. Please pay as soon as possible.",
                type: "payment");
        ...
        {
            _logger.LogInformation(
                "Getting payment overview for landlord {0}", landlordId);

            var props = await _appDbContext.Properties
                .Where(w => w.LandlordId == landlordId)
                .ToListAsync();
            ...
            return new LandlordPaymentsOverviewDto
            {
                ...
                TotalOverdue = summaries
                    .Where(s => s.OverdueCount > 0)
                    .Sum(s => s.Payments
        ...
        public async Task<int> CleanupDuplicatePaymentsAsync()
        {
            ...
            _logger.LogInformation(
                "Removing {0} duplicate payment(s)", duplicates.Count);
            _appDbContext.RentPayments.RemoveRange(duplicates);
            await _appDbContext.SaveChangesAsync();
