...
import { CreateCentreAdminInput } from '@/types/student';
import { useAppSelector } from '@/app/store/store';
import { useRegisterConsultantMutation } from '@/app/errors/authApi';
...
export function AddMemberModal({ onSuccess }: AddMemberModalProps) {
    ...
    const [addConsultant, { isLoading }] = useRegisterConsultantMutation();
    const [errors, setErrors] = useState<string[]>([]);

    const selectedCentreId = useAppSelector((state) => state.trainingCentreStore?.selectedCentreId);

    const [formData, setFormData] = useState<CreateCentreAdminInput>({
        firstName: '',
        lastName: '',
        email: '',
        phoneNumber: '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await addConsultant(submitData).unwrap();
            toast.success('Centre consultant added successfully');
            setIsOpen(false);
            ...
            if (error?.data?.details && error.data.details.length > 0) {
                ...
                toast.error('Failed to add centre consultant. Please try again.');
            }
            console.error('Add centre consultant error:', error);
        }
    };
            ...
            <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
                <form onSubmit={handleSubmit}>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Enter the member's details below to create a new member record.
                        </DialogDescription>
                    </DialogHeader>
